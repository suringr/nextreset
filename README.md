...
- **Command Center homepage**: one verified Next Drop, plus My Games — reordering only, never filtering
- **ONE SHOT // 80 CONTRACTS** at `/play/`: a full-screen sniper game, progress kept on the device, and no ad code on the page
...
│   ├── play/              # The arcade route (generated by scripts/update-play-page.ts)
│   ├── assets/            # styles.v2.css, app.js, player.js, one-shot.js
...
## 🎨 V4: the command center and the arcade
...
```bash
npm run design:apply     # after changing a token
```
...
`render-home.ts` writes the lead block and the cards from the same ordering:

- **Next Drop** is the first card of the `upcoming` group — the soonest verified future-facing event.
  There is no second selection rule that could disagree with the grid below it. A value the pipeline
  states is exact becomes a live countdown in the browser; a day-only value stays a date and says
  `TIME NOT ANNOUNCED`. With nothing upcoming it leads with the latest verified change, under that
  heading; with nothing verified at all, it says so.
- **My Games** reorders, and never filters. The build renders all twelve cards; `app.js` *moves* tracked
  ones into a shelf and puts them back exactly where the build had them when untracked. Without
  JavaScript the page is the complete grid with every value in it.
- The **track control** is a sibling of each card's link, never a child of it, and ships `hidden`
  because without JavaScript it does nothing.

### Local player state

One versioned record under one key, `nextreset.player.v1`: tracked games, and ONE SHOT's progress — XP,
the contracts unlocked, the stars on each, the best full-run score. No account and no server —
deliberately all of it. Storage failure never breaks the site, stored data is validated and clamped
rather than trusted, and nothing in it is ever a published fact: tracking a game changes presentation
order and nothing else, and earns no XP.

The record's format is version 2 since ONE SHOT. Version 1 of `player.js` rebuilt the whole record from its
own defaults when it wrote, so a tab left open across that release, or a rollback, would have deleted the
game's progress; it treats a version-2 record as one from the future and never writes over it.
RESET//SCOPE's records and achievements are carried in the record untouched for the same reason, and
nothing reads them.

The header's player chip is a compact identity — `MARKSMAN · 18,240 XP` — and nothing more. The contract,
the stars and the score belong to the homepage card and to `/play/`.

### The arcade

`/play/` is ONE SHOT // 80 CONTRACTS: eighty sniper contracts in eight chapters — find the red-case
courier, then the armed hostile before he fires — with ammo, reloads, headshots, stars, XP and nine ranks.

**It is the approved prototype, moved over as close to 1:1 as the site allows.** The prototype file is kept
in `scripts/v2/__tests__/fixtures/one-shot/`, and `one-shot.test.ts` rebuilds `public/assets/one-shot.js`
and the page's stylesheet from it and the list of approved edits in `one-shot-approved.ts`; any other
difference fails the build. The edits are:

- **progress in `nextreset.player.v1`** instead of the prototype's own keys — the five places it read or
  wrote them;
- **RELOAD fixed**: the prototype's overlay ended the attempt on the RELOADING message, so a reload
  ...
  and keeps both;
- **a hidden page, or N's prompt, holds the contract**: frames stop in a background tab or behind the
...
- **It carries no AdSense loader**, as a named rule in `carriesLoader` beside the 404 page. It is an
  interactive surface where a mis-tap costs a contract, and Auto ads position anchors and vignettes from
  the account, which this repository cannot see.
- It declares `noindex, follow` and is not submitted. A canvas page is thin by the only measure an
  index applies.
...
```bash
npm run play:apply       # regenerate /play/, then chrome:apply and design:apply
```

### What the build now runs

```
refresh:data -> export:site -> render:pages -> publish dist/
```

`render:pages` additionally strips the served stylesheet of its comments and indentation (the authored
file in `public/` keeps every word) before the asset hashes are taken, so each hash names the bytes a
visitor downloads.
