...
- **RESET//SCOPE arcade** at `/play/`: five missions, local records, and no ad code on the page
...
│   ├── assets/            # styles.v2.css, app.js, player.js, scope*.js
...
and a test enforces it, because a colour written inline is a colour no contrast test can find. Every text
...
One versioned record under one key, `nextreset.player.v1`: tracked games, arcade records, XP,
achievements. No account and no server — deliberately all of it. Storage failure never breaks the site,
stored data is validated and clamped rather than trusted, and nothing in it is ever a published fact:
tracking a game changes presentation order and nothing else.

### The arcade

`/play/` is RESET//SCOPE, a five-mission arcade identification shooter.

- **It carries no AdSense loader**, as a named rule in `carriesLoader` beside the 404 page. It is an
  interactive surface where a mis-tap costs a life, and Auto ads position anchors and vignettes from the
  account, which this repository cannot see.
- It declares `noindex, follow`, so it is crawled and its twelve tracker links are followed, but it is
  not submitted. A canvas page is thin by the only measure an index applies.
- **Its code loads nowhere else.** A test asserts the homepage and all twelve tracker pages reference
  none of `scope-core.js`, `scope.js` or `scope-play.js`.

```
scope-core.js    the rules — no canvas, no DOM, seeded RNG, tested in Node
scope.js         the board — sizing, drawing, update
scope-play.js    the page  — loop, input, HUD, overlays, pause
```

Three plain scripts rather than ES modules on purpose: `_headers` serves `/assets/*.js` immutable for a
year and the build hashes asset URLs it finds **in HTML**, not `import` specifiers inside a JS file — so a
module importing `./engine.js` would be cached for a year at an unversioned URL.
...
```bash
npm run play:apply       # regenerate /play/ from the registry and the game's own tables
```
