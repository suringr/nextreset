...
const HOME = [
    ...
    `.hero{margin:0 0 var(--sp-6)}`,
    `.h1{font-size:var(--size-h1);font-weight:900;letter-spacing:-.02em;margin:0 0 var(--sp-2)}`,
    ...
    `.card{display:block;background:linear-gradient(180deg,var(--sheen),transparent 60%),var(--surface);border:1px solid var(--line);border-radius:var(--radius);padding:var(--sp-4);min-height:132px}`,
